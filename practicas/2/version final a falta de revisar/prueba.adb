...
begin
	...
	--La primera falla porque el denominador es negativo pero no se si hay que tratarlo de alguna forma o eliminarlo o cambiar por -1/2
	--Put("B /= -1/2 ");
	...
	New_Line;
	Put("A/B /= -4/3");
	if A/B /= -4/3 then Error; end if;	--Falla esto porque la division no la debe hacer bien
	Put(": OK");
	New_Line;
	Put("A-B /= 7/6");
	if A-B /= 7/6 then Error; end if;
